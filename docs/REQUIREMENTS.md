...
| 6 | LLM予算 | 初回バックフィルのみ（約$80、一度限り、Sonnet 4.6で全4,000社）。定期更新は当面なし、運用開始後に別途検討 |
...
### 11.4 初回バックフィル（一度限り）
- 4,000社の有報初回取得: IR Collector経由で1日程度（無料）
- 4,000社のSonnet 4.6による初回バックキャスト推定: **約$80・約8時間**
- 推定結果は永続キャッシュとしてDBに保存（自動失効なし）
- 定期更新は実装するが自動実行はしない（手動コマンドで必要時のみ）
- 運用開始後、再推定の頻度・対象は別途検討
